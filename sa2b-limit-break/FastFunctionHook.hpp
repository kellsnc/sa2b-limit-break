...
/// <summary>
/// An usercall function wrapper class using function pointer type.
/// </summary>
/// <typeparam name="FuncType">Function pointer defining the wrapped function.</typeparam>
/// <typeparam name="TRetReg">The return register.</typeparam>
/// <typeparam name="TRegs">Registers of each argument in left to right order.</typeparam>
template<typename FuncType, int TRetReg, int... TRegs>
class UsercallFunctionPtr
{
public:
	
	UsercallFunctionPtr(FuncType address)
	{
		Initialize((intptr_t)address, TRetReg, TRegs...);
	}

	UsercallFunctionPtr(intptr_t address)
	{
		Initialize(address, TRetReg, TRegs...);
	}

	UsercallFunctionPtr() = delete;
	UsercallFunctionPtr(UsercallFunctionPtr&) = delete;
	UsercallFunctionPtr(UsercallFunctionPtr&&) = delete;
...
/// <summary>
/// An userpurge function wrapper class using function pointer type.
/// </summary>
/// <typeparam name="FuncType">Function pointer defining the wrapped function.</typeparam>
/// <typeparam name="TRetReg">The return register.</typeparam>
/// <typeparam name="TRegs">Registers of each argument in left to right order.</typeparam>
template<typename FuncType, int TRetReg, int... TRegs>
class UserpurgeFunctionPtr
{
public:

	UserpurgeFunctionPtr(FuncType address)
	{
		Initialize((intptr_t)address, TRetReg, TRegs...);
	}

	UserpurgeFunctionPtr(intptr_t address)
	{
		Initialize(address, TRetReg, TRegs...);
	}

	UserpurgeFunctionPtr() = delete;
	UserpurgeFunctionPtr(UserpurgeFunctionPtr&) = delete;
	UserpurgeFunctionPtr(UserpurgeFunctionPtr&&) = delete;
...
/// <summary>
/// A thiscall function wrapper class using function pointer type.
/// </summary>
/// <typeparam name="FuncType">Function pointer defining the wrapped function.</typeparam>
/// <typeparam name="TRetReg">The return register.</typeparam>
/// <typeparam name="TRegs">Registers of each argument in left to right order.</typeparam>
template<typename FuncType>
class ThiscallFunctionPtr
{
public:
	using traits = function_traits<FuncType>; // +func decays lambda to function pointer
	using return_type = typename traits::return_type;
	using argument_types = typename traits::argument_types;

	ThiscallFunctionPtr(FuncType address)
	{
		Initialize((intptr_t)address);
	}

	ThiscallFunctionPtr(intptr_t address)
	{
		Initialize(address);
	}

	ThiscallFunctionPtr() = delete;
	ThiscallFunctionPtr(ThiscallFunctionPtr&) = delete;
	ThiscallFunctionPtr(ThiscallFunctionPtr&&) = delete;
...
private:
	...
	{
		Wrapper = (FuncType)GenerateThiscallWrapper(address, traits::arity - 1);
	}
};