...
void Set_Init(const IniFile* config)
{
	if (config->getBool("Limits", "SetList", true))
	{
		...
		WriteData(reinterpret_cast<short**>(0x4889BD), &SetTable[0].Flags);
		WriteData(reinterpret_cast<short**>(0x488A1F), &SetTable[0].Flags);
		WriteData(reinterpret_cast<short**>(0x488AFA), &SetTable[0].Flags);
		WriteData(reinterpret_cast<short**>(0x488B61), &SetTable[0].Flags);
		WriteData(reinterpret_cast<short**>(0x488BFE), &SetTable[0].Flags);
		WriteData(reinterpret_cast<short**>(0x4C7B7E), &SetTable[0].Flags);
